...
local function pNotifyGroup(group, header, msg, icon, colour, length)
    if not group or not EmploymentGroup[group] then return print("Group not found...") end
    for _, v in pairs(EmploymentGroup[group].members) do
        TriggerClientEvent('rj_groups:client:CustomNotification', v.Player,
            header or "NO HEADER",
            msg or "NO MSG"
        )
    end
...
local function DestroyGroup(groupID)
    ...
    exports['rj_groups']:resetJobStatus(groupID)
    TriggerEvent("rj_groups:server:GroupDeleted", groupID, members)

    EmploymentGroup[groupID] = nil
    TriggerClientEvent('rj_groups:client:RefreshGroupsApp', -1, EmploymentGroup)
...
local function setJobStatus(groupID, status, stages)
    ...
    for i=1, #m do
        if m[i] then
            TriggerClientEvent("rj_groups:client:AddGroupStage", m[i], status, stages)
        end
    end
...
local function resetJobStatus(groupID)
    ...
    for i=1, #m do
        if m[i] then
            TriggerClientEvent("rj_groups:client:AddGroupStage", m[i], EmploymentGroup[groupID].status, EmploymentGroup[groupID].stage)
            TriggerClientEvent('rj_groups:client:RefreshGroupsApp', m[i], EmploymentGroup, true)
        end
    end
...
RegisterNetEvent("rj_groups:server:employment_checkJobStauts", function ()
    local src = source
    local checkStatus = GetGroupByMembers(src)
    if checkStatus then
        TriggerClientEvent('rj_groups:client:showEmploymentPage', src)
    else
        TriggerClientEvent('rj_groups:client:showEmploymentGroupPage', src)
    end
end)

RegisterNetEvent("rj_groups:server:jobcenter_CreateJobGroup", function(data)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('rj_groups:server:jobcenter_DeleteGroup', function(data)
    local src = source
...
QBCore.Functions.CreateCallback('rj_groups:server:GetGroupsApp', function(_, cb)
    cb(EmploymentGroup)
end)

RegisterNetEvent('rj_groups:server:jobcenter_JoinTheGroup', function(data)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('rj_groups:server:getAllGroups', function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback('rj_groups:server:jobcenter_CheckPlayerNames', function(_, cb, csn)
    local Names = {}
...
RegisterNetEvent('rj_groups:server:jobcenter_leave_grouped', function(data)
    local src = source
